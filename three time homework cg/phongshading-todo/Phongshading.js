...
// 环境纹理贴图相关变量
var cubeMapTexture; // 立方体贴图纹理
var isEnvironmentMapping = false; // 是否启用环境映射，默认关闭避免问题
var cubeMapLoaded = false; // 环境贴图是否加载完成

// 雾化效果相关变量 - 新增
var fogEnabled = false; // 是否启用雾化
var fogDensity = 0.02; // 雾密度
var fogGradient = 1.5; // 雾梯度
var fogColor = vec3(0.737255, 0.745098, 0.752941); // 雾颜色（与背景色一致）
...
function render() {
	...
	gl.uniform1f(gl.getUniformLocation(program, "u_fogGradient"), fogGradient);
	gl.uniform3fv(gl.getUniformLocation(program, "u_fogColor"), flatten(fogColor));
	gl.uniform3fv(gl.getUniformLocation(program, "u_cameraPos"), flatten(eyePos));
...
window.onkeydown = function (e) {
	...
	switch (code) {
		...
		case 71: // G键增加雾密度
			fogDensity += 0.005;
			console.log("雾密度: " + fogDensity.toFixed(3));
			break;
		case 72: // H键减小雾密度
